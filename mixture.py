...
class Mixture(Material):
	...
	Defined differently than a regular openmc.Material,
	...
	----------
	materials:      iterable
		list of instances of openmc.Material to mix
	vfracs:         iterable
		list of floats describing the volume fraction of each
	...
	def __init__(self, materials, vfracs, material_id=None, frac_type='wo', name=""):
		...
		for i in range(len(materials)):
			...
			for nuc in mat.nuclides:
				nuclide = nuc[0]
				new_wt = wtf*nuc[1]/density
				if nuc in mix_isos:
					old_wt = mix_isos[nuc][1]
